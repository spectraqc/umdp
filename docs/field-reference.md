...
`loudness` carries two ceilings over the whole deliverable, outside `standards[]` because neither is a per-standard target: `true_peak_max` (dBTP) and, from 0.18.0, `short_term_max` (LUFS) — the ceiling on the 3 s sliding-window Short-term Loudness of EBU Tech 3341. EBU R 128 s1 recommends (d) sets it at −18.0 LUFS for short-form content; plain R 128 states none, so a programme profile carrying one is stating a house limit rather than the standard. Absence and `null` both mean no ceiling.

`sync` carries two *different* audio-to-video measurements, and 0.17.0 renamed them to match what is actually measured. **Offset** — `max_offset_ms` (with optional `max_offset_ms_hard_fail_above`) — is a single fixed lip-sync error, constant across the programme: the median per-event audio-vs-video delta, positive when audio lags video. This is the engine-bound threshold, and it is what a spec means by *"AV sync should be within ±10 ms"* or *"sound must not lead or lag the vision by more than 5 ms"*. **Drift** — `max_drift_ms_per_min` — is progressive desync that *accumulates* over the timeline: the slope of that offset against programme time, the signature of a resample or a 25 ↔ 23.976 speed change, which a single median cannot see.

Before 0.17.0 the offset threshold was called `max_drift_ms`. The name was wrong in both directions: it read as a drift bound, while every engine bound it to the median offset and every source spec that set it was quoting a lip-sync figure. It is retired; profiles that carried it now carry `max_offset_ms` with the same value. `must_match_video_duration` asserts the audio runs the full programme duration.
